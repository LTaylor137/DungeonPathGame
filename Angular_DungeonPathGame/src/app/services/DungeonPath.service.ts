...
export class DungeonPathService {
  ...
  randomJoin(lvl: number, rmNo: number, conxRm1: number, maxRm: number) {
    ...
    if (this.rmBlockList.includes(rmNo)) {
      ...
      } else if (Math.random() > 0.8 && rmNo > 1 && rmNo < maxRm) {
        if (Math.random() > 0.6) {
          this.rmBlockList.push(rmNo);
          newconxRm2 = conxRm1 + 1   //    +
          this.crossbackBlock = true;
  ...
  generateMap() {
    ...
    while (this.LevelNo <= this.mapSize) {
      ...
      while (roomNo <= this.maxRmThisLevel && (this.LevelNo <= this.mapSize - 4)) {
        ...
        if (this.LevelNo === 1) {
          thislevel.roomList.push(new Room(this.LevelNo, roomNo, ConnectionRoomNo, ConnectionRoomNo + 1, "first", this.createID(this.LevelNo, roomNo)))
          roomNo += 1;
          ConnectionRoomNo += 2;
          this.maxRmNextLevel += 1;
          this.splitBlock = true;
        } else if ((Math.random() > 0.2) && this.maxRmThisLevel < (this.mapSize / 5)) {
          thislevel.roomList.push(new Room(this.LevelNo, roomNo, ConnectionRoomNo, ConnectionRoomNo + 1, "splithi%", this.createID(this.LevelNo, roomNo)))
          roomNo += 1;
          ConnectionRoomNo += 2;
          this.maxRmNextLevel += 1;
          this.splitBlock = true;
          ...
          if (this.maxRmThisLevel >= (this.mapSize / 2)) {
            // join - or stay
            console.log("this.joinBlock 10 = " + this.joinBlock)
            if ((this.LevelNo > this.mapSize / 6) && roomNo !== 1 && Math.random() > 0.8 && this.joinBlock === false) {
              console.log("this.random true joinBlock 11 = " + this.joinBlock)
              thislevel.roomList.push(new Room(this.LevelNo, roomNo, ConnectionRoomNo - 1, 0, "join1", this.createID(this.LevelNo, roomNo)))
              roomNo += 1;
              this.maxRmNextLevel -= 1;
              this.joinBlock = true;
            } else {
              console.log("this.smax joinBlock 12 = " + this.joinBlock)
              thislevel.roomList.push(new Room(this.LevelNo, roomNo, ConnectionRoomNo,
                this.randomJoin(this.LevelNo, roomNo, ConnectionRoomNo, this.maxRmThisLevel), "Smax", this.createID(this.LevelNo, roomNo)))
              roomNo += 1;
              ConnectionRoomNo += 1;
            ...
            if (Math.random() > 0.8 && this.splitBlock === false) {
              ...
              thislevel.roomList.push(new Room(this.LevelNo, roomNo, ConnectionRoomNo, ConnectionRoomNo + 1, "split", this.createID(this.LevelNo, roomNo)))
              ...
              thislevel.roomList.push(new Room(this.LevelNo, roomNo, ConnectionRoomNo,
                this.randomJoin(this.LevelNo, roomNo, ConnectionRoomNo, this.maxRmThisLevel), "Sroom", this.createID(this.LevelNo, roomNo)))
              roomNo += 1;
              ConnectionRoomNo += 1;
      ...
      while (this.maxRmThisLevel > 0 && (this.LevelNo > this.mapSize - 4)) {
        ...
        if (this.LevelNo === (this.mapSize - 1)) {
          thislevel.roomList.push(new Room(this.LevelNo, roomNo, 1, 0, "2ndlast", this.createID(this.LevelNo, roomNo)))
          roomNo += 1;
          ConnectionRoomNo += 1;
          this.maxRmNextLevel = 1;
          ...
          thislevel.roomList.push(new Room(this.LevelNo, roomNo, ConnectionRoomNo - 1, 0, "join2", this.createID(this.LevelNo, roomNo)))
          roomNo += 1;
          this.maxRmNextLevel -= 1;
          console.log("is it here?")
          this.joinBlock = true;
        } else {
          thislevel.roomList.push(new Room(this.LevelNo, roomNo, ConnectionRoomNo, 0, "Jroom", this.createID(this.LevelNo, roomNo)))
          roomNo += 1;
          ConnectionRoomNo += 1;
